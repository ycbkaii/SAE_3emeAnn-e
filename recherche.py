...
def search_books(query: str = Form(...)):
    ...
    cur.execute("SELECT id_livre, title, description, cover_link FROM masterbook._livre WHERE title ILIKE %s", (f"%{query}%",))
    books = cur.fetchall()
    results["books"] = [{"id": b[0], "title": b[1], "description": b[2] or "Pas de description", "cover": b[3] or ""} for b in books]

    # Recherche des auteurs
    cur.execute("SELECT id_auteur, nom_complet FROM masterbook._auteur WHERE nom_complet ILIKE %s", (f"%{query}%",))
