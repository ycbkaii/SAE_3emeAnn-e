...
for i, genre1 in enumerate(genres):
    ...
    for j, genre2 in enumerate(genres):
        if i != j:
            if device != "cpu" :
               similarity = torch.cosine_similarity(representations[genre1], representations[genre2])
            else :  
                similarity = cosine_similarity(representations[genre1].detach().numpy(), representations[genre2].detach().numpy())
            similaires[genre1].append((genre2, similarity.item()))

print(similaires["Gender"])
pd.DataFrame(similaires).to_csv("simila.csv")
